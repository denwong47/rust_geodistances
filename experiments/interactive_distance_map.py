...
from rust_geodistances import CalculationSettings, haversine
...
if __name__ == "__main__":
    ...
    THRES_KM = CalculationSettings().spherical_radius * THRES
    ...
    def inverse_haversine(start_pos, vector, target_distance):
        ...
        _return = haversine.displace(np.array([curr_pos]), target_distance, _bearing)

        return _return[0, 1], _return[0, 0]
    ...
    def onclick(event):
        ...
        if x is not None and y is not None:
            ...
            distances = haversine.distance_from_point(np.array((y, x)), grid_arr)
